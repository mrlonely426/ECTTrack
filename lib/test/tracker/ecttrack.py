...
from lib.models.mobilevit_track.mobilevitv3_track import build_mobilevitv3_track
...
class MobileViTv3Track(BaseTracker):

    def __init__(self, params, dataset_name):
        super(MobileViTv3Track, self).__init__(params)
        network = build_mobilevitv3_track(params.cfg, training=False)
    ...
    def track(self, image, info: dict = None):
        ...
        """
        cls_map = (torch.squeeze(torch.squeeze(response, 0), 0) * 255).detach().byte().cpu().numpy()
        colored_cls_map = cv2.resize(cv2.cvtColor(cls_map, cv2.COLOR_RGB2BGR), (256, 256))
        #logger.info("colored_cls_map: {},".format(colored_cls_map.shape))
        new_cls_map = colored_cls_map / np.max(colored_cls_map)
        response_map = cv2.applyColorMap(np.uint8(255 * new_cls_map), cv2.COLORMAP_JET)
        image_with_response = cv2.addWeighted(x_patch_arr, 1.0, response_map, 0.7, 0)
        
        #save_path = os.path.join(self.save_dir, "%04d.jpg" % self.frame_id)
        feat_fused = (torch.squeeze(torch.mean(out_dict['feat_fused'],dim=1),0)).detach().byte().cpu().numpy()
        colored_feat_fused = cv2.resize(cv2.cvtColor(feat_fused, cv2.COLOR_RGB2BGR), (256, 256))
        normed_mask = colored_feat_fused / np.max(colored_feat_fused)
        normed_mask = np.uint8(255 * normed_mask)
        normed_mask = cv2.applyColorMap(normed_mask, cv2.COLORMAP_JET)

        cls_map = self.generate_colored_map(out_dict['cls_map'])
        reg_map = self.generate_colored_map(out_dict['reg_map'])
        image_with_cls = cv2.addWeighted(x_patch_arr, 1.0, cls_map, 0.6, 0)
        ...
        if self.debug:
            if not self.use_visdom:
                ...
                self.visdom.register(torch.from_numpy(response_map).permute(2, 0, 1), 'image', 1, 'target_response')
                self.visdom.register(torch.from_numpy(image_with_response).permute(2, 0, 1), 'image', 1, 'image_with_response')
                self.visdom.register(torch.from_numpy(cls_map).permute(2, 0, 1), 'image', 1, 'cls_map')
                self.visdom.register(torch.from_numpy(reg_map).permute(2, 0, 1), 'image', 1, 'reg_map')
                self.visdom.register(torch.from_numpy(image_with_cls).permute(2, 0, 1), 'image', 1, 'image_with_cls')
    ...
    def generate_colored_map(self, attention_map):
        feat_fused = (torch.squeeze(torch.mean(attention_map,dim=1),0)* 255).detach().byte().cpu().numpy()
        colored_feat_fused = cv2.resize(cv2.cvtColor(feat_fused, cv2.COLOR_RGB2BGR), (256, 256))
        normed_mask = colored_feat_fused / np.max(colored_feat_fused)
        normed_mask = np.uint8(255 * normed_mask)
        normed_mask = cv2.applyColorMap(normed_mask, cv2.COLORMAP_JET)
