...
class SeparableSelfAttentionHeadModule(BaseModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.conv_proj_cls = ConvLayer(
            opts=opts["cls"],
            in_channels= 2 * opts["cls"]["in_channels"],
            out_channels=opts["cls"]["in_channels"],
            kernel_size=1,
            stride=1,
            use_norm=True,
        ...
        self.conv_proj_reg = ConvLayer(
            opts=opts["reg"],
            in_channels= 2 * opts["reg"]["in_channels"],
            out_channels=opts["reg"]["in_channels"],
            kernel_size=1,
            stride=1,
            use_norm=True,
        ...
        self.conv0_ctr = conv(cls_in_channel*3, cls_in_channel, freeze_bn=freeze_bn)
        ...
        self.conv0_offset = conv(cls_in_reg*3, cls_in_reg, freeze_bn=freeze_bn)
        ...
        self.conv0_size = conv(cls_in_reg*3, cls_in_reg, freeze_bn=freeze_bn)
    ...
    def forward_spatial(self, x: Tensor, x_tasks=None, *args, **kwargs) -> Tensor:
        x = self.resize_input_if_needed(x)


        x_cls_local = self.pre_ssat_cls(x)
        x_reg_local = self.pre_ssat_reg(x)

        # convert feature map to patches
        if self.enable_coreml_compatible_fn:
            x_cls_patches, x_cls_output_size = self.unfolding_coreml(x_cls_local)
            x_reg_patches, x_reg_output_size = self.unfolding_coreml(x_reg_local)
        else:
            x_cls_patches, x_cls_output_size = self.unfolding_pytorch(x_cls_local)
            x_reg_patches, x_reg_output_size = self.unfolding_pytorch(x_reg_local)
        ...
        x_cls = self.conv_proj_cls(torch.cat((x_cls, x_cls_local), dim=1))
        x_reg = self.conv_proj_reg(torch.cat((x_reg, x_reg_local), dim=1))

        x_cls = torch.cat((x_cls, x_tasks['CLS'],x_tasks['shared']), dim=1)
        x_reg = torch.cat((x_reg, x_tasks['REG'],x_tasks['shared']), dim=1)
        ...
        return score_map_ctr, bbox, size_map, offset_map, x_cls, x_reg

    def forward(
        self, x: Union[Tensor, Tuple[Tensor]], task_feats=None, *args, **kwargs
    ) -> Union[Tensor, Tuple[Tensor, Tensor]]:
        if isinstance(x, Tensor):
            # for image data
            return self.forward_spatial(x,task_feats)
        else:
            raise NotImplementedError
    ...
    def get_score_map(self, x_cls, x_reg):
        ...
        # ctr branch
        x_ctr0 = self.conv0_ctr(x_cls)
        x_ctr1 = self.conv1_ctr(x_ctr0)
        x_ctr2 = self.conv2_ctr(x_ctr1)
        x_ctr3 = self.conv3_ctr(x_ctr2)
        score_map_ctr = self.conv4_ctr(x_ctr3)

        x_offset0 = self.conv0_offset(x_reg)
        x_offset1 = self.conv1_offset(x_offset0)
        x_offset2 = self.conv2_offset(x_offset1)
        x_offset3 = self.conv3_offset(x_offset2)
        score_map_offset = self.conv4_offset(x_offset3)

        x_size0 = self.conv0_size(x_reg)
        x_size1 = self.conv1_size(x_size0)
