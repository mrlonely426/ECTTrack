...
class MobileViTv3_Track_Block(MobileViTBlockv3):
    ...
    def forward_spatial(self, x: Tensor, z: Tensor) -> Tensor:
        ...
        fm_x_conv = self.local_rep(x)
        fm_z_conv = self.local_rep(z)

        # convert feature map to patches
        if self.enable_coreml_compatible_fn:
            patches_x, output_size_x = self.unfolding_coreml(fm_x_conv)
            patches_z, output_size_z = self.unfolding_coreml(fm_z_conv)
        else:
            patches_x, output_size_x = self.unfolding_pytorch(fm_x_conv)
            patches_z, output_size_z = self.unfolding_pytorch(fm_z_conv)

        if self.mixed_attn is True:
            # concatenate search (i.e., x) and template (i.e., z) feature patches
            concatenated_patches = torch.cat((patches_x, patches_z), 3)
            #logger.info(concatenated_patches.shape)
            ...
            # perform feature extraction only (i.e., no feature fusion) using the separable transformer blocks
            for transformer_layer in self.global_rep:
                patches_x, lora_attn_feats = transformer_layer(patches_x)
                patches_z, lora_attn_feats = transformer_layer(patches_z)
        ...
        fm_x = self.conv_proj(torch.cat((fm_x,fm_x_conv), dim=1))
        fm_z = self.conv_proj(torch.cat((fm_z,fm_z_conv), dim=1))
