...
def run(settings):
    ...
    settings.log_file = os.path.join(log_dir, "%s-%s-%s.log" % (settings.script_name, settings.config_name, t))
    ...
    # Create network
    if settings.script_name == 'ecttrack':
        net = build_mobilevitv3_track(cfg)
    else:
