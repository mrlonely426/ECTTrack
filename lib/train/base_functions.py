...
def build_dataloaders(cfg, settings):
    ...
    #logger.info(settings.num_search)
...
def get_optimizer_scheduler(net, cfg):
    train_cls = getattr(cfg.TRAIN, "TRAIN_CLS", False)
    train_lora = getattr(cfg.TRAIN, "ENABLED", True)
    if train_cls:
        ...
        if is_main_process():
            print("Train only Lora, Learnable parameters are shown below.")
            for n, p in net.named_parameters():
                if "lora_" in n or 'box_head' in n:
